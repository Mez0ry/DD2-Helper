...
public:
    ...
    static bool HasButtonKey(const std::string& vbutton_str);
    static VKeys VButtonKeyFromStr(const std::string& vbutton_str);

    static bool IsButtonPressed(VKeys vkey);

    static bool HoldButton(HWND window_descriptor, VKeys key, uint16_t hold_time_ms);
    static bool ReleaseButton(HWND window_descriptor, VKeys key, uint16_t button_released_time_ms);
